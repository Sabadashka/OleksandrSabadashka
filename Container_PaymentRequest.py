...
class ContainerPAYMENT_REQUEST:
    ...
    def read_from_file(self, value="None"):
        ...
        while len(array_values) > 0:
            try:
                ...
                print(message)
                print("Номер " + str(number) + " є дефектним")
            ...
            success += 1
        print("\n" + str(success) + "\033[94m об'єкти були успішно прочитані з файлу: \033[0m" + str(value))
    ...
    def delete(self, ID):
        ...
        for payment in self.__array_of_elements:
            ...
            raise ExceptionPayment("Об’єкт не знайдено або контейнер порожній")
    ...
    def search_in_container(self, search_string):
        ...
        print("Платежі успішно знайдено: " + str(counter_of_found_elements))
        return wanted_payments

    def sort(self, key):
        dictionaryGets = {"1": "get_id", "2": "get_amount", "3": "get_currency", "4": "get_payer_email",
                          "5": "get_payment_request_date", "6": "get_payment_due_to_date", "7": "get_transaction_id"}
        try:
            self.__snapshot()
            ...
            print("Контейнер не можна сортувати")
            raise ExceptionPayment("Key " + key + " is missing")

        print("Контейнер було успішно відсортовано за атрибутом: " + key)

    def edit(self, ID, key, value):
        for payment in self.__array_of_elements:
            if str(payment.get_id()) == str(ID):
                dictionarySets = {"1": payment.set_id, "2": payment.set_amount, "3": payment.set_currency,
                                  "4": payment.set_payer_email,
                                  "5": payment.set_payment_request_date, "6": payment.set_payment_due_to_date,
                                  "7": payment.set_transaction_id}
                try:
                    self.__snapshot()
                    dictionarySets[key](value)
                except KeyError:
                    self.__snapshots.pop()
                    raise ExceptionPayment("Ключ " + key + " втрачено")

                if self.__file_name != "None":
                    self.write_to_file(self.__file_name)
                print("Атрибут успішно відредаговано")
                return
        raise ExceptionPayment("Елемент з ідентифікатором " + str(ID) + " відсутній")
